...
class LogSystem <T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = logs->logs.stream().filter(obj->obj.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor =logs->logs.stream().filter(obj->obj.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));
